...
def compare_time(uTime,curTime):
    if uTime["date"] == curTime["date"]: # compare dates
        if uTime["time"] == curTime["time"]: #if true compare time
            return uTime["operation"] #if true will return desired operation.
...
def light_closing(maxlig):
    lig= int(get_luxData())
    led_on()
    while lig > maxlig: # this is the loop to open blind
        motor_close() #operate blind for 1 second
        #sleep(0.5)
        lig= int(get_luxData())
        cd = get_distance() # get the new height of the blind
...
while True:
    ...
    op = threading.Thread(target=opening_blind)
    cl = threading.Thread(target=closeing_blind)

    lightop = threading.Thread(target=light_opening,args=[mnl])
    lightcl = threading.Thread(target=light_closing,args=[mxl])
    ...
    if mode == "auto":
        ...
        if cmaxt == True:
            if blindStatus == "open":
                print("maximum temperature reached closing blind")
                cl.start()
                flagcl = True
                blindStatus = "close"  #updates current blind status on device
        ...
        if cmint == True:
            if blindStatus == "close":
                print("minimum temperature reached opening blind")
                op.start()
                flagop = True
                blindStatus = "open"  #updates current blind status on device
        ...
        if cmaxl == True:
            if blindStatus == "open":
                print("maximum light reached closing blind")
                lightcl.start()
                lflagcl = True
                blindStatus = "close"  #updates current blind status on device
        ...
        if cminl == True:
            if blindStatus == "close":
                print("minimum light reached opening blind")
                lightop.start()
                lflagop = True
                blindStatus = "open"  #updates current blind status on device
    ...
    print("current blind status",blindStatus)
    sleep(1.5)
